...
export class ProvincesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.titleService.setTitle('Quản Lý Khách Sạn - Tỉnh Thành');
  ...
  getData(request?: any, isReset = false): void {
    ...
    this.provincesService.get(param).subscribe((data) => {
  ...
  confirm(id: any): void {
    this.isLoading = true;
    this.provincesService.delete(id).subscribe((result) => {
  ...
  openModal(data?: any): void {
    const modal = this.modal.create({
      ...
      nzContent: ProvincesModalComponent,
