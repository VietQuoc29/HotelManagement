...
namespace HotelManagerFull.Web.Controllers
{
    ...
    public class RoomCategoriesController : ControllerBase
    {
        ...
        public RoomCategoriesController(IRoomCategoriesBusiness roomCategoriesBusiness)
        {
        ...
        public async Task<IActionResult> GetAllAsync([FromQuery] PagingRequest request)
        {
            try
            {
                ...
                var result = await _roomCategoriesBusiness.GetAllAsync(request);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpPost("Save")]
        public async Task<IActionResult> SaveAsync([FromForm] RoomCategoriesRequest request)
        {
            try
            {
                if (!ModelState.IsValid || null == request)
                ...
                return Ok(await _roomCategoriesBusiness.SaveAsync(request, false));
            }
            catch (Exception ex)
            {
                return StatusCode(Convert.ToInt32(HttpStatusCode.InternalServerError), ex.ToString());
        ...
        public async Task<IActionResult> DeleteAsync(long id)
        {
            try
            {
                var request = new RoomCategoriesRequest
                {
                    Id = id
                };
                ...
                return Ok(await _roomCategoriesBusiness.SaveAsync(request, true));
            }
            catch (Exception ex)
            {
                return StatusCode(Convert.ToInt32(HttpStatusCode.InternalServerError), ex.ToString());
        ...
        public IActionResult Suggestion()
        {
            try
            {
                var result = _roomCategoriesBusiness.Suggestion();
                return Ok(result);
            }
            catch (Exception ex)
            {
